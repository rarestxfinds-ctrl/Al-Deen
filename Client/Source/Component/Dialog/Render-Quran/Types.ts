...
// Client-safe static layout bounds mapped to replace active local file exports
...
// Global local memory segment array fallback index
let cachedSegmentsMap: Record<number, PageSegment[]> | null = null;

// Populates structural mushaf mappings asynchronously over safe environment infrastructure
export async function prefetchAllPageSegments(): Promise<void> {
  try {
    const res = await fetch(`${BACKEND_BASE_URL}/api/quran/all-segments`);
    if (res.ok) {
      cachedSegmentsMap = await res.json();
    }
  } catch {
    cachedSegmentsMap = {};
  }
...
export function pageFontFamily(font: RenderFont, surahId: number, verseNumber: number): string | undefined {
  if (font === "uthmani_v1" || font === "uthmani_v2") {
    if (cachedSegmentsMap) {
      for (let pageNum = 1; pageNum <= 604; pageNum++) {
        const segs = cachedSegmentsMap[pageNum];
        if (!segs) continue;
...
export function buildEmbedPreviewDoc(
  ...
  const cardFor = (v: any): string => {
    ...
    const trBlocks = cfg.translations.filter((t: string) => t !== "None").map((src: string) =>
      `<div class="tr" style="color:${cols.translationCol};font-size:${cfg.translationSize}px">${escapeHtml(extraTr[src]?.[v.verseNumber - 1] ?? "")}</div>`
    ).join("");
    const tlBlocks = cfg.transliterations.filter((t: string) => t !== "None").map((src: string) =>
