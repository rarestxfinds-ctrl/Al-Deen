...
export async function getAidCorpus() {
  ...
  try {
    const filePath = path.resolve(__dirname, "..", "..", "Asset", "Corpus", "Aid.json");
...
export async function getBranches(): Promise<Branch[]> {
  const corpus = await getAidCorpus();
  return corpus.branches;
}

export async function getBranch(id: string): Promise<Branch | undefined> {
  const branches = await getBranches();
  return branches.find(b => b.id.toLowerCase() === id.toLowerCase());
}

export async function getAllSchools(): Promise<(School & { branchId: string; branchName: string })[]> {
  const branches = await getBranches();
  const results: (School & { branchId: string; branchName: string })[] = [];
  for (const branch of branches) {
    for (const school of branch.schools) {
      results.push({ ...school, branchId: branch.id, branchName: branch.name });
    }
  }
  return results;
}

export async function getSchool(id: string): Promise<(School & { branchId: string; branchName: string }) | undefined> {
  const schools = await getAllSchools();
  return schools.find(s => s.id.toLowerCase() === id.toLowerCase());
}